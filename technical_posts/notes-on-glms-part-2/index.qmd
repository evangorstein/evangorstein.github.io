...
Continuing where we left off in Chapter 3 of @wood2017generalized, we begin with a discussion of residuals in GLMs.
...
df = data.frame(x1 = X[, 1],
                ...
                x3 = X[, 3],
                y = y)
pois_mod = glm(y ~ x1 + x2 + x3, family = poisson, data = df)

```

```{r, echo=FALSE, fig.cap="Residuals from a Poisson GLM with log-link"}
par(mfrow=c(1,2))
plot(predict(pois_mod), resid(pois_mod, type="pearson"),
     xlab="Linear predictor", ylab="Pearson residual")
abline(h=0)
plot(predict(pois_mod), resid(pois_mod, type="deviance"),
...
From @eq-resid and @eq-working, we can see that the working residuals coincide with the Pearson residual whenever
...
```{r, include=FALSE}
phi = 0.5
ygam = rgamma(n, shape = 1/phi, scale = mu*phi)
df$ygam = ygam
gam_mod = glm(ygam ~ x1 + x2 + x3, family = Gamma(link="log"), data = df)
```

```{r, echo=FALSE, fig.cap="Residuals from a Gamma GLM with log-link"}
par(mfrow=c(1,3))
plot(predict(gam_mod), resid(gam_mod, type="pearson"), 
     ylim = c(-2.1,2),
     xlab="Linear predictor", ylab="Pearson residual")
abline(h=c(0,sqrt(phi),-sqrt(phi)))
plot(predict(gam_mod), resid(gam_mod, type="deviance"),
     ylim = c(-2.1,2), yaxt = "n",
     xlab="Linear predictor", ylab="Deviance residual")
abline(h=c(0,sqrt(phi),-sqrt(phi)))
plot(predict(gam_mod), resid(gam_mod, type="working"),
     ylim = c(-2.1,2), yaxt = "n",
     xlab="Linear predictor", ylab="Working residual")
abline(h=c(0,sqrt(phi),-sqrt(phi)))
```
...
Consider an observation $y_i$ of a random variable with mean $\mu_i$ and variance $\phi V(\mu_i)$. The log quasi-likelihood for $\mu_i$ given this observations is defined to be
$$
...
$$
is, up to an additive constant not depending on $\mu_i$, the log-likelihood for $\mu_i$ from an observation of $y_i \sim N(\mu_i, \phi)$, the exponential family whose mean-variance relationship is constant. Similarly, for $V(z)=z$, we have 

\begin{align}
\int_{y_i}^{\mu_i} \frac{y_i-z}{\phi z} dz &= \frac{1}{\phi}\left[y_i\log(z) - z\right]_{z=y_i}^{z=\mu_i} \\ &= \frac{y_i\log(\mu_i) - \mu_i}{\phi} + C
\end{align}
is, up to an additive constant not depending on $\mu_i$, the log-likelihood for $\mu_i$ from an observation of $y_i \sim \text{Pois}(\mu_i)$ if we set $\phi$ to 1.
